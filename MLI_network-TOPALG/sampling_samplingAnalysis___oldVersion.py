...
def patch_MLI(MLIsample, SynapseObj):
    ...
    for i in ls:
        MLIgraph.add_node(MLIsample[i[0]][i[1]].index)
        for j in ls:
            w = SynapseObj.w[MLIsample[i[0]][i[1]].index, MLIsample[j[0]][j[1]].index].tolist() # the weight is non zero only if there is a chemical synapse which is directed
            if w!=[] and w!=[0.0]:
                MLIgraph.add_edge(MLIsample[i[0]][i[1]].index, MLIsample[j[0]][j[1]].index, weight=1) # directed graph DiGraph() : the edge is directed
...
def triadsPatterns_distribution_corrector(Nsample, Ntriads, SynapseObj, MLItable,
                                             ...
                                             Dis102=4.0 / 173,
                                             Dis111D=0.0 / 173, Dis111U=5.0 / 173, Dis120C=1.0 / 173, Dis120D=0.0 / 173,
                                             Dis120U=3.0 / 173, Dis201=0.0 / 173, Dis210=0.0 / 173, Dis300=0.0 / 173, iteration=0, previous_newConnectionsStore=0):
    ...
    while 1:
        ...
        while 1:
            ...
            for stepBIS in range(len(memory)):
                ...
                if triadPatPreBIS in decPat:
                    # if a pattern to lower is this triad pattern  ####### NOTE : no need to create the elif situation, because if some patern is underrepresented it means that an other one is too much represented...
                    print(">2nd screening : pattern to lower !")
                    ...
                    while 1:  # The same while loop as the second while loop of the first for loop... But which considers all kinds of patterns, not only the unswotchables ones !
                        ...
                        i = MLI_graph.adj.keys()[KEY1]  # i=presynaptic previous synapse neuron index
                        j = MLI_graph.adj.keys()[KEY2]  # j=postsynaptic previous synapse neuron index
                        print(">>2nd screening : Is there a OR no synapse from ", i, " to ", j, " ?")
        ...
        ### Plot a histogram at each Try
        patList = []
        patValList = []
        for pat in memoryDis:
            patList.append(pat)
            patValList.append((AimDis[pat] * 100, memoryDis[pat] * 100))
        patDataFrame = pd.DataFrame(data=patValList, index=patList, columns=['Aimed dist. -- ' + str(meanDiff), 'Network dist.'])
        ax = patDataFrame.plot.bar()  # s is an instance of Series
        fig = ax.get_figure()
        fig.savefig('/Users/maximebeau/Desktop/Science/4_Stage_UCL_WIBR_HAUSSER/My_project/experiments_results/personnalCon_noPKJ/figures/motifs distributions/duringRun/wholeDebugNet_duringCorrection_Iteration' + str(iteration) + '_Try' + str(Try) + '.jpeg')
...
def iterative_corrections(Niterations, Ncorrection, NtriadsCorrection, Nanalysis, NtriadsAnalysis, SynapseObj_real,
    ...
    while iteration < Niterations:
        ...
        if iteration == 1:  # There is a preCorrection analysis only for the first iteration
            ...
            Analysis_preCorrection = Nsample_sampling__Ntriads_patchings__triadsAnalysis(Nsample=Nanalysis,
                                                                                          Ntriads=NtriadsAnalysis,
                                                                                          SynapseObj=SynapseObj_real,
                                                                                          MLItable=MLItable,
                                                                                          strategy=strategy,
                                                                                          coordUnit=coordUnit,
                                                                                          Ysizeum=Ysizeum,
            ...
            nextpreCor_meanDiff = preCor_meanDiff
            patList = []
            patValList = []
            ...
            fig = ax.get_figure()
            fig.savefig(
                '/Users/maximebeau/Desktop/Science/4_Stage_UCL_WIBR_HAUSSER/My_project/experiments_results/personnalCon_noPKJ/figures/motifs distributions/wholeNet_before_afterCorrection-sampledNet_afterRun/wholeNet_iteration' + str(
            ...
            preCor_meanDiff = nextpreCor_meanDiff
        ...
        correctorOutput = triadsPatterns_distribution_corrector(Nsample=Ncorrection, Ntriads=NtriadsCorrection, SynapseObj=SynapseObj_real,
                                                                MLItable=MLItable,
                                                                TRY=TRY, prev_aim_diff=prev_aim_diff,
                                                                ...
                                                                Dis300=Dis300, iteration=iteration, previous_newConnectionsStore=prev_nCS)
        ...
        # this resets SynapseObj between each iteration
        ...
        ax = patDataFrame.plot.bar()  # s is an instance of Series
        fig = ax.get_figure()
        fig.savefig('/Users/maximebeau/Desktop/Science/4_Stage_UCL_WIBR_HAUSSER/My_project/experiments_results/personnalCon_noPKJ/figures/motifs distributions/wholeNet_before_afterCorrection-sampledNet_afterRun/sampledNet_iteration'+str(iteration)+'_forCorrection.jpeg')
        ...
        Analysis_postCorrection = Nsample_sampling__Ntriads_patchings__triadsAnalysis(Nsample=Nanalysis, Ntriads=NtriadsAnalysis,
                                                                                    SynapseObj=SynapseObj, MLItable=MLItable,
                                                                                    strategy=strategy, coordUnit=coordUnit, Ysizeum=Ysizeum,
                                                                                    Xsizeum=Xsizeum)
        ...
        ########################## Choose to effectively modify or not the SynapseObj_real as SynapseObj is ##################################
        ...
        if Analysis_postCorrection[3] < preCor_meanDiff:  # compare the meanDiffs
            ...
            nextpreCor_meanDiff = Analysis_postCorrection[3]  # the new lower wholeNetwork meanDiff is Analysis_postCorrection[3]
            ...
            print("\n\nApparently the meanDiff after the iteration "+str(iteration)+" is higher than after the last one. No plotting of the post analysis.")
    ...
    fig = ax.get_figure()
    fig.savefig(
        '/Users/maximebeau/Desktop/Science/4_Stage_UCL_WIBR_HAUSSER/My_project/experiments_results/personnalCon_noPKJ/figures/motifs distributions/wholeNet_before_afterCorrection-sampledNet_afterRun/meanDiff_iterationsEvolution.jpeg')
